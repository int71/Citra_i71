...
//            handlers->winding_setter();
            I71_handlers.winding_setter();
        }
        for (std::size_t i = 0; i < buffer.size(); ++i) {
//            handlers->vertex_handler(buffer[i]);
            I71_handlers.vertex_handler(buffer[i]);
        }
    }
}
...
//    emitter.handlers->vertex_handler = vertex_handler;
    emitter.I71_handlers.vertex_handler = vertex_handler;
//    emitter.handlers->winding_setter = winding_setter;
    emitter.I71_handlers.winding_setter = winding_setter;
}
