...
FramebufferLayout DefaultFrameLayout(u32 width, u32 height, bool swapped, bool upright) {
	...
	if constexpr(true){
		constexpr auto			cu32screenwidth0=u32(Core::kScreenTopWidth);
		constexpr auto			cu32screenheight0=u32(Core::kScreenTopHeight);
		constexpr auto			cu32screenwidth1=u32(Core::kScreenBottomWidth);
		constexpr auto			cu32screenheight1=u32(Core::kScreenBottomHeight);
		constexpr auto			cu32screengap=u32(Core::I71_kScreenGap);
		constexpr auto			cu32screenwidth=std::max({cu32screenwidth0,cu32screenwidth1});
		constexpr auto			cu32screenheight=cu32screenheight0+cu32screenheight1;
		const auto				cu32multiply=std::max({
			u32(1),
			std::min({
				((
					Settings::values.I71_stereoscopy.GetValue()&&
					(
						(Settings::values.render_3d.GetValue()==Settings::StereoRenderOption::SideBySide_Parallel)||
						(Settings::values.render_3d.GetValue()==Settings::StereoRenderOption::SideBySide_Cross)
					)
				)?(width-I71_cu32StereoGap)/2:width)/cu32screenwidth,
				height/cu32screenheight
			})
		});
		const auto				cu32renderwidth0=cu32screenwidth0*cu32multiply;
		const auto				cu32renderheight0=cu32screenheight0*cu32multiply;
		const auto				cu32renderwidth1=cu32screenwidth1*cu32multiply;
		const auto				cu32renderheight1=cu32screenheight1*cu32multiply;
//		const auto				cu32renderwidth=cu32screenwidth*cu32multiply;
		const auto				cu32renderheight=cu32screenheight*cu32multiply;
		const auto				cu32rendergap=std::min({height-cu32renderheight,cu32screengap*cu32multiply});
		const auto				cu32renderx0=(width-cu32renderwidth0)/2;
		const auto				cu32rendery0=(height-cu32renderheight-cu32rendergap)/2;
		const auto				cu32renderx1=(width-cu32renderwidth1)/2;
		const auto				cu32rendery1=cu32rendery0+cu32renderheight0+cu32rendergap;
		...
		return {
			...
			true,
			{
				cu32renderx0,
				cu32rendery0,
				cu32renderx0+cu32renderwidth0,
				cu32rendery0+cu32renderheight0
			},{
				cu32renderx1,
				cu32rendery1,
				cu32renderx1+cu32renderwidth1,
				cu32rendery1+cu32renderheight1
			},
			!upright
		};
	}else{
