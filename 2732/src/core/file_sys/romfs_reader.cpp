...
std::size_t DirectRomFSReader::ReadFile(std::size_t offset, std::size_t length, u8* buffer) {
	...
	if(I71_iIsEnableROMCaching()){
		if(!I71_pu8Cache){
			const auto				i71_cszncache=static_cast<std::size_t>(data_size);

			I71_pu8Cache.reset(new u8[i71_cszncache]);
			file.ReadAtBytes(I71_pu8Cache.get(),i71_cszncache,file_offset);
			if(is_encrypted){
				CryptoPP::CTR_Mode<CryptoPP::AES>::Decryption	d(key.data(),key.size(),ctr.data());

				d.Seek(crypto_offset);
				d.ProcessData(I71_pu8Cache.get(),I71_pu8Cache.get(),i71_cszncache);
			}
		}
		memcpy(buffer,I71_pu8Cache.get()+offset,length);
		return length;
	}
