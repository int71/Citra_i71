...
ResultStatus			AppLoader_THREEDSX::ReadProgramId(
	u64&					out_program_id
){
	std::vector<u8>			vu8icon;

	//	generate pseudo id from title string
	if(ReadIcon(vu8icon)==ResultStatus::Success){
		if(sizeof(SMDH)<=vu8icon.size()){
			const auto				cpcsmdhtemp=(const SMDH*)vu8icon.data();
			const auto				cpac16title=cpcsmdhtemp->GetShortTitle(SMDH::TitleLanguage::Japanese);
			u8						au8programid[]={0,0,0,0};

			for(int i=0;i<int(cpac16title.size());++i){
				const auto				cc16current=cpac16title[i];
				const auto				cszioffset=i&(int(sizeof(au8programid))-1);

				au8programid[cszioffset]^=u8(cc16current&0xff)^u8(cc16current>>8);
			}
			out_program_id=
				0x0000000000000000|
				(u64(au8programid[0])<<24)|
				(u64(au8programid[1])<<16)|
				(u64(au8programid[2])<< 8)|
				(u64(au8programid[3])<< 0);
			return ResultStatus::Success;
		}
	}
	return ResultStatus::Error;
...
ResultStatus AppLoader_THREEDSX::ReadTitle(std::string& title) {
	title=I71_strGetTitle(filepath.c_str());
