...
bool					Thread::I71_eIsSerializable(void)const{
	return HLERequestContext::I71_steIsSerializable(wakeup_callback.get());
}
...
bool					ThreadManager::I71_eIsSerializable(void)const{
	for(const auto& cthcurrent:wakeup_callback_table)if(!cthcurrent.second->I71_eIsSerializable())return false;
	return true;
}
