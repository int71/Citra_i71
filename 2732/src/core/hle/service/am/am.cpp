...
InstallStatus InstallCIA(const std::string& path,
    ...
    if (container.Load(path) == Loader::ResultStatus::Success) {
		...
		{
			const auto				i71_cstrtitlepath=GetTitlePath(
				Service::AM::GetTitleMediaType(container.GetTitleMetadata().GetTitleID()),
				container.GetTitleMetadata().GetTitleID()
			);
			if(FileUtil::Exists(i71_cstrtitlepath+"/content")){
				const auto				i71_cstraliasfilename=fmt::format("{}content/alias.txt",i71_cstrtitlepath);
				FileUtil::IOFile		i71_file(i71_cstraliasfilename,"wb");

				if(!i71_file.IsOpen()){
					LOG_ERROR(Service_AM,"Could not open CIA alias i71_file '{}'.",i71_cstraliasfilename);
					return InstallStatus::ErrorFailedToOpenFile;
				}else{
					std::string				i71_strfilename;

					Common::SplitPath(path.c_str(),NULL,&i71_strfilename,NULL);
					i71_file.WriteString(i71_strfilename.c_str());
					i71_file.Close();
				}
			}
		}
