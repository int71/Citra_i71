...
static GMainWindow*		I71_pqwMain;
float					I71_fnBallanceFront;
float					I71_fnBallanceCenter;
float					I71_fnBallanceSide;
float					I71_fnBallanceBack;
QFont					I71_fontUI;

void					I71_UpdateSettings(void){
	I71_pqwMain->I71_UpdateSettings();
	I71_pqwMain->update();
	return;
}

extern "C" int			I71_iIsEnableNetwork(void){
	const auto&				cvalues=UISettings::values;

	return int(cvalues.I71_enable_network.GetValue());
}

extern "C" int			I71_iIsEnableROMCaching(void){
	const auto&				cvalues=UISettings::values;

	return int(cvalues.I71_enable_rom_caching.GetValue());
}

void					I71_SetDesktopFPS(void){
	const auto&				cvalues=UISettings::values;

	if(
		const auto				cinfps=cvalues.I71_inDesktopFPS;
		cinfps
	){
		static DEVMODE			devmodenew;

		devmodenew.dmSize=sizeof(devmodenew);
		devmodenew.dmFields=DM_DISPLAYFREQUENCY;
		devmodenew.dmDisplayFrequency=cinfps;
		ChangeDisplaySettingsEx(NULL,&devmodenew,NULL,CDS_FULLSCREEN,NULL);
	}
	return;
}

QStringList				I71_qstrlistGetFontFamiles(void){
	QHash<QString,int>		qhashfontfamily;
	QStringList				qstrlistreturn;
	LOGFONT					lftemp;
	static const auto		stcfpicallback=[](
		const LOGFONT*			cplfsource,
		const TEXTMETRIC*		cptmsource,
		const DWORD				cdwcfonttype,
		const LPARAM			clprmobject
	)->int{
		auto&					qhashfontfamily=*(QHash<QString,int>*)(clprmobject);

		if(
			(cplfsource->lfFaceName[0]!=L'@')&&
			((cdwcfonttype&RASTER_FONTTYPE)==0)
		)if(
			const auto				cqstrfontfamily=QString::fromWCharArray(cplfsource->lfFaceName);
			QFont(cqstrfontfamily).exactMatch()
		)qhashfontfamily[cqstrfontfamily]=0;
		return 1;
	};

	lftemp.lfCharSet=DEFAULT_CHARSET;
	lftemp.lfFaceName[0]=0;
	lftemp.lfPitchAndFamily=0;
	EnumFontFamiliesEx(GetDC(NULL),&lftemp,stcfpicallback,LPARAM(&qhashfontfamily),0);
	qstrlistreturn.append(QStringLiteral("(Default)"));
	{
		auto					qstrlisttemp=qhashfontfamily.keys();

		qstrlisttemp.sort();
		for(const auto cqstrkey:qstrlisttemp)qstrlistreturn.append(cqstrkey);
	}
	return qstrlistreturn;
}

void					I71_SetUIFont(
	QWidget* const			cpqwdgthis,
	const int				cinsizeadjust
){
	const auto				cinpoint=(UISettings::values.I71_inUIFontPoint*(cinsizeadjust+16))>>4;

	I71_fontUI.setPointSize(cinpoint);
	cpqwdgthis->setFont(I71_fontUI);
	return;
}
...
GMainWindow::GMainWindow(Core::System& system_)
    ...
    ui->setupUi(this);
	I71_fontUI=QFont();
	I71_fontUI.setStyleStrategy(QFont::PreferQuality);
	I71_UpdateSettings();
    statusBar()->hide();
...
//    const QString fullscreen = QStringLiteral("Fullscreen");
	const QString fullscreen = QStringLiteral("Toggle Fullscreen");
...
/*    connect_shortcut(QStringLiteral("Exit Fullscreen"), [&] {
	...
	connect_shortcut(
		QStringLiteral("Toggle 3D"),
		[]{
			Settings::values.I71_stereoscopy=!Settings::values.I71_stereoscopy;
		}
	);
...
void GMainWindow::BootGame(const QString& filename) {
	I71_Render_UpdatePauseState(false);
...
//            QFileInfo(UISettings::values.recent_files[i]).fileName());
			QString::fromStdString(Loader::I71_strGetTitle(UISettings::values.recent_files[i].toStdString().c_str())));
...
void GMainWindow::OnPauseGame() {
	I71_Render_UpdatePauseState(true);
...
void GMainWindow::ToggleFullscreen() {
	...
	I71_Render_UpdateFullscreenState();
/*    if (ui->action_Fullscreen->isChecked()) {
        ShowFullscreen();
    } else {
        HideFullscreen();
...
void GMainWindow::HideMouseCursor() {
    if (!emu_thread || !emu_thread->IsRunning()|| !UISettings::values.hide_mouse.GetValue()) {
...
//        setWindowTitle(QStringLiteral("Citra %1").arg(full_name));
		setWindowTitle(QStringLiteral("Citra/i71"));
    } else {
		QString					qstrtitle;

		if(render_window->I71_eIsPaused())qstrtitle=QStringLiteral("Citra/i71 - %1 - Paused").arg(game_title);
		else qstrtitle=QStringLiteral("Citra/i71 - %1").arg(game_title);
...
void					GMainWindow::I71_UpdateSettings(void){
	if(UISettings::values.I71_qstrUIFontFamily.length())I71_fontUI.setFamily(UISettings::values.I71_qstrUIFontFamily);
	I71_fontUI.setWeight(QFont::Thin);
	{
		const auto&				cvalues=Settings::values;
		const auto				cfnvolume=powf(10.0f,float(cvalues.I71_volume.GetValue())*.05f);

		I71_fnBallanceFront=powf(10.0f,float(cvalues.I71_ballance_front.GetValue())*.05f)*cfnvolume;
		I71_fnBallanceCenter=powf(10.0f,float(cvalues.I71_ballance_center.GetValue())*.05f)*cfnvolume;
		I71_fnBallanceSide=powf(10.0f,float(cvalues.I71_ballance_side.GetValue())*.05f)*cfnvolume;
		I71_fnBallanceBack=powf(10.0f,float(cvalues.I71_ballance_back.GetValue())*.05f)*cfnvolume;
	}
	{
		const auto&				cvalues=UISettings::values;
		const auto				enetwork=cvalues.I71_enable_network.GetValue();
	
		if(!enetwork){
			if(ui->action_Leave_Room->isEnabled())ui->action_Leave_Room->trigger();
		}
		ui->action_View_Lobby->setEnabled(enetwork);
		ui->action_Start_Room->setEnabled(enetwork);
		ui->action_Leave_Room->setEnabled(false);
		ui->action_Connect_To_Room->setEnabled(enetwork);
		ui->action_Show_Room->setEnabled(false);
	}
	return;
}

void					GMainWindow::I71_stSetFullscreenSize(
	QWidget* const			cpqwthis
){
	const auto				cqpwindow=cpqwthis->pos();
	const auto				cqrcwindow=cpqwthis->frameGeometry();
	const auto				cqrcclient=cpqwthis->geometry();
	auto					pqscreen=QGuiApplication::screenAt(cqpwindow);

	if(pqscreen==NULL)pqscreen=QGuiApplication::screens()[0];
	{
		const auto				cqrcscreen=pqscreen->geometry();
...
void					GMainWindow::I71_Render_UpdatePauseState(
	const bool				cepause
){
	const auto				cefullscreen=ui->action_Fullscreen->isChecked();
	const auto				cesinglewindowmode=ui->action_Single_Window_Mode->isChecked();
	...
	if(cepause){
		...
		if(render_window->I71_eIsPaused()){
			render_window->I71_UpdatePauseState(false);
			if(cefullscreen){
				if(cesinglewindowmode){
					ui->menubar->hide();
					showFullScreen();
...
void					GMainWindow::I71_Render_UpdateFullscreenState(void){
	const auto				cefullscreen=ui->action_Fullscreen->isChecked();
	const auto				cesinglewindowmode=ui->action_Single_Window_Mode->isChecked();
	...
	if(render_window->I71_eIsPaused()){
		...
		if(cefullscreen)ShowFullscreen();
		else HideFullscreen();
	}
	I71_Render_eFullscreen=cefullscreen;
	return;
}
