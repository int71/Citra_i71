...
extern void				I71_SetUIFont(QWidget* const cpqwdgthis,const int cinsizeadjust=0);
...
//    }
	...
	if(item_model->rowCount()){
		tree_view->setCurrentIndex(
			item_model->index(
				int(UISettings::values.I71_u16iGameListSelectedItem),
				0,
				item_model->index(int(UISettings::values.I71_u16iGameListSelectedGroup),0)
			)
		);
	}
	SetFilterFocus();
...
void GameList::AddGamePopup(QMenu& context_menu, const QString& path, const QString& name,
    ...
    connect(uninstall_game, &QAction::triggered, this, [this, name, media_type, program_id] {
        QMessageBox::StandardButton answer = QMessageBox::question(
            this, tr("Citra/i71"), tr("Are you sure you want to uninstall '%1'?").arg(name),
            QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
        if (answer == QMessageBox::Yes) {
            std::vector<std::tuple<Service::FS::MediaType, u64, QString>> titles;
            titles.emplace_back(media_type, program_id, name);
    ...
    connect(uninstall_dlc, &QAction::triggered, this, [this, name, dlc_program_id] {
        QMessageBox::StandardButton answer = QMessageBox::question(
            this, tr("Citra/i71"), tr("Are you sure you want to uninstall all DLC for '%1'?").arg(name),
            QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
        if (answer == QMessageBox::Yes) {
            std::vector<std::tuple<Service::FS::MediaType, u64, QString>> titles;
            titles.emplace_back(Service::FS::MediaType::SDMC, dlc_program_id,
...
void					GameList::I71_UpdateFont(void){
	I71_SetUIFont(tree_view);
	I71_SetUIFont(tree_view->header(),-4);
	return;
}
...
void GameList::SaveInterfaceLayout() {
	...
	UISettings::values.I71_u16iGameListSelectedGroup=tree_view->currentIndex().parent().row();
	UISettings::values.I71_u16iGameListSelectedItem=tree_view->currentIndex().row();
}
