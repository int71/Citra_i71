...
GRenderWindow::GRenderWindow(QWidget* parent_, EmuThread* emu_thread_, Core::System& system_,
                             bool is_secondary_)
    : QWidget(parent_), EmuWindow(is_secondary_), emu_thread(emu_thread_), system{system_}, I71_ePaused(false) {
...
void					GRenderWindow::I71_UpdatePauseState(
	const bool				cepause
){
	if(I71_ePaused!=cepause){
		I71_ePaused=cepause;
		if(cepause){
			I71_qpxPausedImage=QGuiApplication::screens()[0]->grabWindow(winId());
...
void					GRenderWindow::paintEvent(
	QPaintEvent* const		cpqpeevent
){
	if(I71_ePaused){
		const auto				cqrcclient=cpqpeevent->rect();
		const auto				cqszpause=I71_qpxPausedImage.size();
		const auto				cqptpause=QPoint(
			(cqrcclient.width()-cqszpause.width())/2,
			(cqrcclient.height()-cqszpause.height())/2
		);
		QPainter				painter(this);
		...
		painter.setBrush(QBrush(
			QColor(
				int(Settings::values.bg_red.GetValue()*255.0f),
				int(Settings::values.bg_green.GetValue()*255.0f),
				int(Settings::values.bg_blue.GetValue()*255.0f)
			),Qt::SolidPattern
		));
		painter.drawRect(QRect(0,0,cqrcclient.width(),cqrcclient.height()));
		painter.drawPixmap(
			QRect(
				cqptpause.x(),
				cqptpause.y(),
		...
		painter.setBrush(QBrush(QColor(0,0,0,191),Qt::SolidPattern));
		painter.drawRect(QRect(0,0,cqrcclient.width(),cqrcclient.height()));
	}
	return;
}
