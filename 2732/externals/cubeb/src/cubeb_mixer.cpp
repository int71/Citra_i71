...
extern float			I71_fnBallanceFront;
extern float			I71_fnBallanceCenter;
extern float			I71_fnBallanceSide;
extern float			I71_fnBallanceBack;
...
MixerContext::auto_matrix()
{
  ...
  cubeb_channel_layout out_ch_layout = clean_layout(_out_ch_layout);
	...
	if constexpr(true){
		const auto				cdnfront=double(I71_fnBallanceFront);
		const auto				cdncenter=double(I71_fnBallanceCenter);
		const auto				cdnside=double(I71_fnBallanceSide);
		const auto				cdnback=double(I71_fnBallanceBack);

		matrix[FRONT_LEFT][FRONT_LEFT]=cdnfront;
		matrix[FRONT_RIGHT][FRONT_RIGHT]=cdnfront;
		matrix[FRONT_CENTER][FRONT_LEFT]=.5*cdncenter;
		matrix[FRONT_CENTER][FRONT_RIGHT]=.5*cdncenter;
		matrix[SIDE_LEFT][FRONT_LEFT]=cdnside;
		matrix[SIDE_RIGHT][FRONT_RIGHT]=cdnside;
		matrix[BACK_LEFT][FRONT_LEFT]=cdnback;
		matrix[BACK_RIGHT][FRONT_RIGHT]=cdnback;
	}else{
